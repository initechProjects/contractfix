...
exports.sendMailInvitation = function(user, senderemail, sendername, title, token) {
  var mailbody = `<p>You have received this invitation from ${sendername}.</p><p>You can clicking on the link below to review, comment and sign ${title}.<br/><a href='http://${Config.server.host}:${Config.server.port}/${Config.email.invitationUrl}/${token}'>Verification Link</a></p>`;
  // var mailbody = Welcome.email(user, token);
