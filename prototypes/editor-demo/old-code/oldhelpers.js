...
DocumentModel.prototype.update = function(content) {
  var diffs = dmp.diff_main(this.original, content);
...
DocumentModel.prototype.acceptAt = function(index) {
  var diffStart = 0;
  var diffEnd = this.content().length;
  for (var i = this.diffs.length - 1; i >= 0; i--) {
    diffStart = diffEnd - this.diffs[i][1].length + 1;
    if (index >= diffStart && index <= diffStart + this.diffs[i][1].length) {
      this.accept(i);
      break;
    }
    diffEnd -= this.diffs[i][1].length;
  }
  this.updateOriginal();
};


DocumentModel.prototype.acceptRange = function(start, end) {
  var diffStart = 0;
  var diffEnd = this.content().length;
  for (var i = this.diffs.length - 1; i >= 0; i--) {
    var content = this.diffs[i][1];
    ...
    if (start >= diffStart && start <= diffEnd)
      this.accept(i);
    if (start <= diffStart && end >= diffEnd)
      this.accept(i);
    if (end >= diffStart && end <= diffEnd)
      this.accept(i);
...
DocumentModel.prototype.acceptAll = function() {
  this.each(function(diff, i, diffStart, diffEnd) {
    this.accept(i);
  });
  this.updateOriginal();
};

DocumentModel.prototype.rejectAt = function(index) {
  var diffStart = 0;
  var diffEnd = this.content().length;
  for (var i = this.diffs.length - 1; i >= 0; i--) {
    var content = this.diffs[i][1];
    diffStart = diffEnd - content.length;
    if (index >= diffStart && index <= diffStart + content.length) {
      this.reject(i);
      break;
    }
    diffEnd -= content.length;
...
DocumentModel.prototype.rejectRange = function(start, end) {
  this.each(function(diff, i, diffStart, diffEnd) {
    if (start >= diffStart && start <= diffEnd)
      this.reject(i);
    if (start <= diffStart && end >= diffEnd)
